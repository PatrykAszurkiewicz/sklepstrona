...
function OrdersAdminPage() {
  ...
  if (orders.length === 0) {
    return <h1 className="text-center pt-4">Brak zamówień</h1>;
  }
      ...
      <tr>
        ...
        <td>
          {status === "processing" ? (
            ...
            <Badge bg="success">Wysłane</Badge>
          )}
        </td>
        <td>
          <span
          ...
          >
            Zobacz zamówienie <i className="fa fa-eye"></i>
          </span>
        </td>
      </tr>
    );
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        {orderToShow.map((order) => (
          <div className="order-details__container d-flex justify-content-around py-2">
            ...
            <p>Cena: ${(Number(order.price) * order.count).toFixed(2)}</p>
          </div>
        ))}
        <Modal.Footer>
